...
def handle_im(im, bboxes):
    ...
    scale = tf.concat([[sh], [sw], [sh], [sw]], axis=0)
...
def img_scale(im, bboxes):
    ...
    yxyx = tf.concat([[y], [x], [y], [x]], axis=0)
    yxyx = tf.to_float(yxyx)
    bboxes = tf.concat([bboxes[..., :4] * scale + yxyx, bboxes[..., 4:]], axis=-1)
...
def crop_img_tf(img, bboxes):
    ...
    h1 = tf.to_float(h1)
    h2 = tf.to_float(h2)
    y1 = tf.maximum(h1, bboxes[:, 0:1])
    x1 = tf.maximum(w1, bboxes[:, 1:2])
    y2 = tf.minimum(h2 - 1, bboxes[:, 2:3])
    x2 = tf.minimum(w2 - 1, bboxes[:, 3:4])
    ...
    bboxes = tf.concat([y1, x1, y2, x2, bboxes[:, 4:5]], axis=1)
...
def parse(se):
    ...
    #yxyx:
    # read_Data与read_Data_2的不同：
    # read_Data
    # bboxes先转成xyxy的形式再处理。
    # read_Data_2
    # yxyx格式处理完之后，最后再转成xyxy的形式。
